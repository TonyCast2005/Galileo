...
func _ready():
    ...
    http.request("https://galileo-af640-default-rtdb.firebaseio.com/OpcionMultiple/programacion_basica.json")
...
func cargar_pregunta(indice: int):
    # ⭐ Restaurar botón de pista al cargar una nueva pregunta
    boton_pista.visible = true
...
func mostrar_retroalimentacion(texto: String, color: Color):
    ...
    # ✔ Solo un ciclo de texto letra por letra
    for c in texto:
        label_retro.text += c
